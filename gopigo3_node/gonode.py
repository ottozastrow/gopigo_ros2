...
class Robot:
    ...
    def __init__(self):
        ...
        node = rclpy.create_node("gopigo3")
        

        self.br = TransformBroadcaster()

        # subscriber
        node.create_subscription(Int16, "motor/dps/left", lambda msg: self.g.set_motor_dps(self.ML, msg.data), 10)
        node.create_subscription(Int16, "motor/dps/right", lambda msg: self.g.set_motor_dps(self.MR, msg.data), 10)
        node.create_subscription(Int8, "motor/pwm/left", lambda msg: self.g.set_motor_power(self.ML, msg.data), 10)
        node.create_subscription(Int8, "motor/pwm/right", lambda msg: self.g.set_motor_power(self.MR, msg.data), 10)
        node.create_subscription(Int16, "motor/position/left", lambda msg: self.g.set_motor_position(self.ML, msg.data), 10)
        node.create_subscription(Int16, "motor/position/right", lambda msg: self.g.set_motor_position(self.MR, msg.data), 10)
        node.create_subscription(Int16, "servo/pulse_width/1", lambda msg: self.g.set_servo(self.S1, msg.data), 10)
        node.create_subscription(Int16, "servo/pulse_width/2", lambda msg: self.g.set_servo(self.S2, msg.data), 10)
        node.create_subscription(Float64, "servo/position/1", lambda msg: self.set_servo_angle(self.S1, msg.data), 10)
        node.create_subscription(Float64, "servo/position/2", lambda msg: self.set_servo_angle(self.S2, msg.data), 10)
        node.create_subscription("cmd_vel", Twist, self.on_twist, 10)

        node.create_subscription(UInt8, "led/blinker/left", lambda msg: self.g.set_led(self.BL, msg.data), 10)
        node.create_subscription(UInt8, "led/blinker/right", lambda msg: self.g.set_led(self.BR, msg.data), 10)
        node.create_subscription(ColorRGBA, "led/eye/left", lambda c: self.g.set_led(self.EL, int(c.r*255), int(c.g*255), int(c.b*255)), 10)
        node.create_subscription(ColorRGBA, "led/eye/right", lambda c: self.g.set_led(self.ER, int(c.r*255), int(c.g*255), int(c.b*255)), 10)
        node.create_subscription(ColorRGBA, "led/wifi", lambda c: self.g.set_led(self.EW, int(c.r * 255), int(c.g * 255), int(c.b * 255)), 10)

        # publisher
        self.pub_enc_l = node.create_publisher(Float64, 'motor/encoder/left', 10)
        self.pub_enc_r = node.create_publisher(Float64, 'motor/encoder/right', 10)
        self.pub_battery = node.create_publisher(Float64, 'battery_voltage', 10)
        self.pub_motor_status = node.create_publisher(MotorStatusLR, 'motor/status', 10)
        self.pub_odometry = node.create_publisher(Odometry, "odom", 10)
        self.pub_joints = node.create_publisher(JointState, "joint_state", 10)

        # services
        self.srv_reset = node.create_service(Trigger, 'reset', self.reset)
        self.srv_spi = node.create_service(SPI, 'spi', lambda req: SPIResponse(data_in=self.g.spi_transfer_array(req.data_out)))
        self.srv_pwr_on = node.create_service(Trigger, 'power/on', self.power_on)
        self.srv_pwr_off = node.create_service(Trigger, 'power/off', self.power_off)

        # main loop
        rate = rclpy.Rate(node.declare_parameter('hz', 30))   # in Hz
